"use strict";
//console.log('Running from Outside Script');
var VERSION = "1.8.0",
...
Twitter.prototype.getRequestToken = function() {
	return new Promise((resolve,reject)=>{
		this.oa.getOAuthRequestToken({x_auth_access_type: this.x_auth_access_type}, (error, oauthToken, oauthTokenSecret, results) => {
			if (error) {
				reject({error});
			} else {
...
Twitter.prototype.getAuthUrl = function(requestToken, options) {
	...
	}if(options && options.include_email){
		extraArgs += "&include_email=true";
	}
...
Twitter.prototype.getAccessToken = function(requestToken, requestTokenSecret, oauth_verifier) {
	return new Promise((resolve,reject)=>{
		this.oa.getOAuthAccessToken(requestToken, requestTokenSecret, oauth_verifier, function(error, oauthAccessToken, oauthAccessTokenSecret, results) {
			if (error) {
				reject({error})
...
Twitter.prototype.verifyCredentials = function(accessToken, accessTokenSecret, params) {
	...
	return new Promise((resolve,reject)=>{
		this.oa.get(url, accessToken, accessTokenSecret, (error, data, response) => {
			if (error) {
				reject({error});
				//callback(error);
			} else {
...
// 				break;
// 			case "upload_media":
// 				this.uploadMedia(params, accessToken, accessTokenSecret).then(res =>{
// 					console.log('Media resolve',res);
// 					resolve(res)
// 				}).catch(err => {
// 						reject(err);
// 					});
// 				// try{
// 				// 	let uploadMedia = this.uploadMedia(params, accessToken, accessTokenSecret);
...
Twitter.prototype.uploadMediaV2 = function(params, accessToken, accessTokenSecret) {
	var parameter = (params.isBase64) ? "media_data" : "media";
	var value = (params.isBase64) ? params.media : fs.createReadStream(params.media);
	var options = {
		method: 'POST',
		url: uploadBaseUrl + "media/upload.json",
		oauth: {
...
Twitter.prototype.users = function(type, params, accessToken, accessTokenSecret) {
	...
	return new Promise((resolve,reject)=>{
		if (method == "GET") {
			this.oa.get(baseUrl + "users/" + url + ".json?" + querystring.stringify(params), accessToken, accessTokenSecret, (error, data, response) => {
				if (error) {
					const requestUrl = baseUrl + "users/" + url + ".json?" + querystring.stringify(params);
					reject({
						error,
						data,
						response,
			...
			this.oa.post(baseUrl + "users/" + url + ".json", accessToken, accessTokenSecret, params, (error, data, response) => {
				if (error) {
					//callback(error, data, response);
					reject({
						error,
...
Twitter.prototype.friends = function(type, params, accessToken, accessTokenSecret) {
	var url = type.toLowerCase(); // ids or list
	return new Promise((resolve,reject)=>{
		this.oa.get(baseUrl + "friends/" + url + ".json?" + querystring.stringify(params), accessToken, accessTokenSecret, (error, data, response) => {
			if (error) {
				const apiUrl = baseUrl + "friends/" + url + ".json?" + querystring.stringify(params);
				reject({
					apiUrl,
					error,
					data,
...
Twitter.prototype.followers = function(type, params, accessToken, accessTokenSecret) {
	var url = type.toLowerCase(); // ids or list
	return new Promise((resolve,reject)=>{
		this.oa.get(baseUrl + "followers/" + url + ".json?" + querystring.stringify(params), accessToken, accessTokenSecret, function(error, data, response) {
			if (error) {
				const apiUrl = baseUrl + "folllowers/" + url + ".json?" + querystring.stringify(params);
				reject({
					apiUrl,
					error,
					data,
